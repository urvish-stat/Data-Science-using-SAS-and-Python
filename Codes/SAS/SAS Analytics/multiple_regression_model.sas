...
proc reg data = fitness;
  ...
  title "Predicting Oxygen_Consumption from Performance and RunTime";
quit;
  
** Variable Selection using rsquare, adjrsq and cp;  
ods graphics / imagemap = on;
proc reg data = fitness plots(only) = (rsquare adjrsq cp);
  ...
  title "Best Models using All Regression Options";
quit;

** All possible regression using Cp;
proc reg data = fitness plots(only) = (cp);
  ALL_REG: model Oxygen_Consumption = Performance RunTime age weight 
                                      run_pulse rest_pulse maximum_pulse
