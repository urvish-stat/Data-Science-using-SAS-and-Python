...
*  PROGRAM NAME : Examining Distribution.sas
*  PURPOSE      : Distribution of categorical variables 	
******************************************************************;
...
** Frequency for categorical variables Survived,Gender,Class,
   Gender by Survived and Class by Survived;
proc freq data = titanic;
   tables
   /*Frequency for one categorical variables*/
   survived gender class
   /*Frequency for more than one categorical variables*/
   gender*survived class*survived / plots(only) = freqplot(scale = percent);
   format survived survfmt.;
run;

** TABLES Statement options:
   FREQPLOT<suboptions> - requests the frequency plot. Frequency Plots are available for 
                          frequency (one-way) and crosstabulation (n-way) tables. For 
                          multiway tables, PROC FREQ provides a two-way frequency plots
                          for each stratum. 

   (SCALE=) - specifies the scale of the frequencies to display. The default is 
              SCALE=FREQ, which displays the unscaled frequencies. SCALE=PERCENT
              displays percentages (relative frequencies);

** Histogram for Age variable across the level of Survived;
