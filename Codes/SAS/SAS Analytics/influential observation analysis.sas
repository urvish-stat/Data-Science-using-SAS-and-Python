...
ods output residualbypredicted = rstud
           cooksdplot = cook
           dffitsplot = dffits
           dfbetaspanel = dfbs;
proc reg data = fitness 
         plots(only label) = (residualbypredicted /*Residual by Predicted values*/ 
                        cooksd /*Cooks'D Plot*/
                        dffits /*DFFITS Plot*/
                        dfbetas /*DFBETAS Plot*/);
  Predict: model Oxygen_Consumption = RunTime age  run_pulse maximum_pulse;
  id name; /*This is to identify observarion based on name of person*/
  title "Influential Observation Analysis";
quit;