...
proc freq data = titanic;
  tables (gender class) * survived / 
       chisq /* Calculate the chi-square test */
       expected /* Calculate the expected cell frequenies */
       cellchi2 /* Display the contribution of cell in total chi-square statistics */
       relrisk /* Calculate the odds ratios and probability (risk) ratios */;
run;
...
proc freq data = exact;
  tables a*b / chisq nocol nopercent expected cellchi2;
  title "Exact P-Values";
run;

** Test for Ordinal Association;
proc freq data = titanic;
  tables class*survived / chisq measures cl;
  title "Ordinal Association between Class and Survival";
run;
